...
void MicrostrainParser::parse_imu_packet(const mscl::MipDataPacket& packet)
{
  ...
  if (has_accel)
  {
    ...
    std::copy(m_config->m_imu_linear_cov.begin(), m_config->m_imu_linear_cov.end(),
              m_publishers->m_imu_msg.linear_acceleration_covariance.begin());
  }

  if (has_gyro)
  {
    // Since the sensor does not produce a covariance for angular velocity, set it based on our pulled in parameters.
    std::copy(m_config->m_imu_angular_cov.begin(), m_config->m_imu_angular_cov.end(),
              m_publishers->m_imu_msg.angular_velocity_covariance.begin());
  }

  if (has_quat)
  {
    // Since the MIP_AHRS data does not contain uncertainty values we have to set them based on the parameter values.
    std::copy(m_config->m_imu_orientation_cov.begin(), m_config->m_imu_orientation_cov.end(),
              m_publishers->m_imu_msg.orientation_covariance.begin());
  }
...
void MicrostrainParser::parse_filter_packet(const mscl::MipDataPacket& packet)
{
  ...
  for (mscl::MipDataPoint point : points)
  {
    switch (point.field())
    {
      ...
      case mscl::MipTypes::CH_FIELD_ESTFILTER_ESTIMATED_ORIENT_QUATERNION:
      {
        ...
        m_publishers->m_filter_relative_pos_msg.pose.pose.orientation =
            m_publishers->m_filter_msg.pose.pose.orientation;
      }
      break;
      ...
      case mscl::MipTypes::CH_FIELD_ESTFILTER_ESTIMATED_NED_UNCERT:
      {
        if (point.qualifier() == mscl::MipTypes::CH_NORTH)
        {
          ...
          if (m_config->m_use_enu_frame)
          {
            ...
            m_publishers->m_filter_relative_pos_msg.twist.covariance[0] =
                m_publishers->m_filter_msg.twist.covariance[0];
          }
          else
          {
            m_publishers->m_filter_msg.twist.covariance[7] = pow(m_curr_filter_vel_uncert_north, 2);
            m_publishers->m_filter_relative_pos_msg.twist.covariance[7] =
                m_publishers->m_filter_msg.twist.covariance[7];
          }
        }
        else if (point.qualifier() == mscl::MipTypes::CH_EAST)
        {
        ...
        else if (point.qualifier() == mscl::MipTypes::CH_DOWN)
        {
          ...
          m_publishers->m_filter_relative_pos_msg.twist.covariance[14] =
              m_publishers->m_filter_msg.twist.covariance[14];
        }
      }
      break;
      ...
      case mscl::MipTypes::CH_FIELD_ESTFILTER_GNSS_DUAL_ANTENNA_STATUS:
      {
        ...
        else if (point.qualifier() == mscl::MipTypes::CH_STATUS)
        {
          ...
          m_publishers->m_gnss_dual_antenna_status_msg.rcv_1_valid =
              status_flags & mscl::InertialTypes::DualAntennaStatusFlags::DATA_VALID_REC_1;
          m_publishers->m_gnss_dual_antenna_status_msg.rcv_2_valid =
              status_flags & mscl::InertialTypes::DualAntennaStatusFlags::DATA_VALID_REC_2;
          m_publishers->m_gnss_dual_antenna_status_msg.antenna_offsets_valid =
  ...
  std::copy(m_config->m_imu_linear_cov.begin(), m_config->m_imu_linear_cov.end(),
            m_publishers->m_filtered_imu_msg.linear_acceleration_covariance.begin());
  std::copy(m_config->m_imu_angular_cov.begin(), m_config->m_imu_angular_cov.end(),
            m_publishers->m_filtered_imu_msg.angular_velocity_covariance.begin());
...
void MicrostrainParser::parse_gnss_packet(const mscl::MipDataPacket& packet, int gnss_id)
{
  ...
  m_publishers->m_gnss_time_msg[gnss_id].header.stamp = ros_time_now(m_node);
    ...
    switch (point.field())
    {
      ...
      case mscl::MipTypes::CH_FIELD_GNSS_2_LLH_POSITION:
      {
        ...
        else if (point.qualifier() == mscl::MipTypes::CH_HORIZONTAL_ACCURACY)
        {
          ...
          m_publishers->m_gnss_msg[gnss_id].position_covariance[4] =
              m_publishers->m_gnss_msg[gnss_id].position_covariance[0];
          m_publishers->m_gnss_odom_msg[gnss_id].pose.covariance[0] =
              m_publishers->m_gnss_msg[gnss_id].position_covariance[0];
          m_publishers->m_gnss_odom_msg[gnss_id].pose.covariance[7] =
              m_publishers->m_gnss_msg[gnss_id].position_covariance[0];
        }
        else if (point.qualifier() == mscl::MipTypes::CH_VERTICAL_ACCURACY)
        {
          ...
          m_publishers->m_gnss_odom_msg[gnss_id].pose.covariance[14] =
              m_publishers->m_gnss_msg[gnss_id].position_covariance[8];
        }
...
void MicrostrainParser::print_packet_stats()
{
  ...
  if (m_config->m_inertial_device->features().supportsCommand(mscl::MipTypes::Command::CMD_DEVICE_STATUS))
  {
    if (m_config->m_inertial_device->features().supportedStatusSelectors().size() > 1)
    {
      ...
      MICROSTRAIN_DEBUG_THROTTLE(m_node, 1.0, "%u IMU (%u errors) Packets", m_imu_valid_packet_count,
      ...
      MICROSTRAIN_DEBUG_THROTTLE(
          m_node, 1.0, "%u FILTER (%u errors)    %u IMU (%u errors)    %u GPS (%u errors) Packets",
          m_filter_valid_packet_count, m_filter_timeout_packet_count, m_imu_valid_packet_count,
          m_imu_timeout_packet_count + m_imu_checksum_error_packet_count, m_gnss_valid_packet_count[GNSS1_ID],
