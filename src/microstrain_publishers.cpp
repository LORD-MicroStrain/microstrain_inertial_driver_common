...
bool MicrostrainPublishers::configure_publishers()
{
  ...
  if (m_config->m_publish_imu &&
      m_config->m_inertial_device->features().supportsCommand(mscl::MipTypes::Command::CMD_MAG_HARD_IRON_OFFSET))
  {
  ...
  MICROSTRAIN_INFO(m_node, "checking if we should publish filter data %d %d", m_config->m_publish_filter,
                   m_config->m_supports_filter);
  if (m_config->m_publish_filter && m_config->m_supports_filter)
  {
    ...
    {
      m_gnss_dual_antenna_status_pub =
          create_publisher<GNSSDualAntennaStatusMsg>(m_node, "nav/dual_antenna_status", 100);
    }
